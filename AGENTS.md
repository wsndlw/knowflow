...
本项目（knowflow）是面向企业/机构的 AI 知识库系统，使用 Codeg 协调 Claude Code、Codex 与 Gemini 串行协作开发。

**本文件是 Claude Code、Codex 和 Gemini 的唯一协作规则来源**（CC 经 `CLAUDE.md` 的 `@AGENTS.md` 引用、Codex 经 `.codex/config.toml` 读取；Gemini 如在当前目录运行，也必须先读取并遵守本文件）。不要在 `CLAUDE.md`、`GEMINI.md` 或其他文件维护另一套可能冲突的规则。
...
| Agent           | 职责                                                                                                    | 可改                                                                                          | 不可越界                                                         |
| --------------- | ------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| **Claude Code** | 需求澄清、PRD、任务拆解、API 契约设计、验收标准、任务交接、最终审查                                     | 文档、PRD、Trellis spec、任务文件、接口契约说明、小范围联调修正                               | 不作为主要前端/后端实现方；不大改后端核心或正式前端页面          |
...
| **Gemini**      | 前端页面、UI 组件、页面路由、交互状态、前端 API 调用、浏览器端验证、视觉/多模态辅助                     | 前端代码、UI 组件、页面路由、样式、前端状态管理、前端 API 调用层                              | 不改后端核心、数据库 schema、权限/RAG 逻辑；不越过 CC 定产品边界 |
| **Codeg**       | 多 Agent 工作台与任务调度                                                                               | —                                                                                             | —                                                                |
| **Trellis**     | PRD、任务、spec、上下文沉淀中心                                                                         | —                                                                                             | —                                                                |

CC 是主控、任务拆解与最终审查方，Codex 是后端工程实现负责人，Gemini 是前端工程实现负责人。**不同 Agent 不要同时修改同一个文件；子任务要有明确输入、输出、修改边界。**
...
多套互相隔离的 skill 系统，各 Agent 只能用自己平台可用的那套：**Trellis skills**（`.agents/skills/trellis-*`，CC、Codex 与 Gemini 共用或按平台注入，负责流程控制）、**Claude Code skills**（CC 内置，需求/审查/前端质量）、**Codex skills**（`~/.codex/skills/`，后端/工程）、**Gemini 可用 skills/agents**（前端实现、浏览器验证、视觉/多模态）。下表是强制路由：场景匹配时必须调用对应 skill；如当前平台没有同名 skill，必须调用最接近的同领域 skill，并在交接/报告中说明替代项，不能静默跳过。

流程主线（共用）：需求/拆任务/实现/检查/沉淀走 `.trellis/workflow.md` 的 Phase 1→2→3；下面的领域 skill 在各 Phase 内叠加使用。

**Claude Code（需求/拆解/审查）**

| 场景                       | 必须调用的 Skill/路由                                                            |
| -------------------------- | -------------------------------------------------------------------------------- |
| 新需求、需求不清、任务拆解 | `grill-with-docs` + `trellis-brainstorm`，结论必须落到 `prd.md`（见裁决①）       |
| API 契约、跨前后端任务拆分 | `trellis-before-dev` 或平台等价上下文加载；必要时阅读 Cross-Layer Thinking Guide |
| 最终审查、DoD 核对         | `trellis-check` + `code-review`；前端视觉/体验问题加 `impeccable`                |
| 浏览器端复核               | `webapp-testing`/`playwright` 或平台等价浏览器验证技能                           |
...
| 场景                          | 必须调用的 Skill                                                                              |
| ----------------------------- | --------------------------------------------------------------------------------------------- |
| 写代码前（常驻底座）          | `trellis-before-dev` + `karpathy-guidelines`（见裁决②）                                       |
...
| PDF 处理                      | `pdf`                                                                                         |
| 接口/API 验证                 | `webapp-testing` + `playwright`；Codex 环境跑不通浏览器时用 `build` + HTTP/API 验证替代并说明 |
| 碰权限/安全代码后（必过）     | `security-best-practices`（见裁决③）                                                          |

**Gemini（前端实现/浏览器验证/视觉）**

| 场景                           | 必须调用的 Skill/路由                                                                      |
| ------------------------------ | ------------------------------------------------------------------------------------------ |
| 写前端组件/页面/路由前         | `trellis-before-dev` 或平台等价上下文加载 + `react-best-practices` + `next-best-practices` |
| 前端接 API、状态管理、错误态   | `trellis-before-dev` + Cross-Layer Thinking Guide 或平台等价前后端契约技能                 |
| UI 质量、视觉打磨、交互细节    | `impeccable` 或 Gemini 平台等价前端/视觉审查技能                                           |
| 浏览器端到端验证、截图、控制台 | `webapp-testing` + `playwright` 或 Gemini 平台等价浏览器自动化技能                         |
| 前端验收前自查                 | `web-design-guidelines` 或平台等价前端验收技能                                             |

上表是硬性路由。**如某 skill 名称在当前平台不可用或显示名不一致，使用最接近的认证/权限/RAG/测试/前端 skill，并在报告中写明替代关系；不能因为名字对不上就跳过该环节**——重要的是该场景有对应能力的 skill 介入，而非名字精确匹配。
...
> **浏览器端到端验证由 Gemini 负责，CC 最终审查可复核**：Codex 进程的 bash 环境通常访问不到 Windows 的 npx / wrapper 路径，跑不通 Playwright，这是环境隔离不是错误。Codex 用 `build` + HTTP 接口验证（curl/cookie jar）+ 后端脚本替代即可，并在报告中如实标注「浏览器验收留给 Gemini/CC」；Gemini 在前端实现阶段跑浏览器验证，CC 在最终审查阶段按风险补验证。

**四个必守裁决：**

1. **grill 必须落到 prd.md**：`grill-with-docs` 产出 `CONTEXT.md`（术语表），而 Codex/Gemini 接活只读 `prd.md` 与明确交接材料。grill 只是 `trellis-brainstorm` 的前置磨刀，需求最终一定要写进 `prd.md`，否则实现方读不到。
...
4. **前端 skills 是 Gemini 强制关卡**：Gemini 写正式前端页面/组件前必须加载前端实现技能，验收前必须加载前端验收或浏览器验证技能；缺失同名 skill 时必须说明替代项。
...
Codex 改 API 后同步类型与说明；Gemini 改前端调用后确认契约未破坏；CC 在最终审查时核对 PRD、API 契约、前端真实调用与 DoD。
