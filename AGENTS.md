...
## 14.5 功能开发流程（分支 / 提交 / 审查 / PR）

每个功能或模块都走以下流程。这是本项目协作的主干，CC 与 Codex 都必须遵守。
...
4. 一个功能/模块完成 → git commit     # ← 提交粒度见下
5. 多个功能/模块攒够 → git push       # ← push 节奏见下
6. gh pr create → 对方在 PR 上审查    # ← 第二次审：留痕 + 把关
7. 用户拍板 → gh pr merge 合入 main
8. git checkout main && git pull      # 切回主干，进下一个功能
```

### 两次审查（都要，顺序固定）

审查发生两次，时机和目的不同，不可省其一：

| | 第一次审（本地） | 第二次审（PR 上） |
|---|---|---|
| 时机 | 写完、**提交之前** | push 后、**合并之前** |
| 在哪 | 本地工作树，看 `git diff` | GitHub PR，看 `gh pr diff` |
| 目的 | 趁早抓 bug，改起来便宜 | 留下 review 记录 + 合并前最终把关 |
| 谁审 | 对方 Agent（Codex 写→CC 审；CC 写→Codex 审） | 同左 |
| 工具 | CC：`code-review` skill；Codex：`security-best-practices` 等 | `gh pr diff` + 把意见贴到 PR |

**核心原则：代码必须先经本地审查、改干净，才能提交。提 PR 时代码应已是「我方认为 OK」的状态——PR 审是第二道关与留痕，不是第一次发现问题的地方。**

### 提交粒度（commit）

- **一个功能或一个模块完成，就 commit 一次。** 不要写了几行就提交，也不要把好几个不相关的功能堆在一个 commit 里。
- 一个 commit 应是一个**逻辑完整、可独立描述**的改动单元（例如「实现文档上传接口」「完成知识库列表页」）。
- commit message 用中文，遵循 `feat:` / `fix:` / `chore:` / `docs:` 前缀约定（与现有历史一致）。
- 每次 commit 前必须已通过本地审查与基本验证（lint / type-check / 关键流程）。

### push 节奏

- **commit 是本地的、频繁的；push 是远程的、成批的。** 二者节奏不同，不要每 commit 都 push。
- 允许在一个分支上累积**多个功能 / 多个模块的 commit**，攒到一个阶段性节点再**一次性 push**。
- push 后再 `gh pr create` 发起 PR，让对方 Agent 审查。
- 例外：如果该分支需要让另一个 Agent 在远程协作、或需要尽早留痕，可提前 push。

### 分支约定

- 分支名用英文：`feat/<功能>`（新功能）、`fix/<问题>`（修复）、`refactor/<范围>`（重构）。
- 一个分支聚焦一类工作，不要在一个分支里混入不相关的功能。
- 真正需要两个 Agent 并行改重叠区域时，用 git worktree + 各自分支，最后 merge。

### push 与合并权限

- **push：谁写谁 push，只 push 自己的功能分支。** push 是「我这部分干完、对外发布等审查」的信号，只有负责方知道何时到节点；不要替对方 push。
- **合并到 main：必须由用户拍板。** 不论分支是谁写的，AI 都不自行 `gh pr merge`。流程是「对方 Agent 审查 → 把结论交给用户 → 用户点头 → 才合并」。push 自己的分支是低风险（分支隔离），合并 main 是高风险（影响共享主干与对方的工作基线），二者权限不同。

### 串行交接（单机共享工作树）

本项目 CC 与 Codex 共用同一个本地工作树，同一时刻只能 checkout 一个分支。因此默认**串行交接**，不并行：

- 同一时间只有一个 Agent 在一个分支上干活。例如 Codex 写完后端 → commit / push → 审查 → 合并 main → CC 再切到自己分支写前端。
- 切换分支会替换整个工作树的文件，会打断另一方正在写的内容——所以不要在另一个 Agent 占用工作树时切分支。
- 仅当确实需要两个 Agent 同时改重叠区域时，才开 git worktree 真正并行（注意：worktree 文件夹里没有 `.trellis/.codex/.agents` 脚手架，Trellis 脚本需在主工作树运行）。
